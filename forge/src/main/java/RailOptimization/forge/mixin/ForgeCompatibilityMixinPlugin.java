package RailOptimization.forge.mixin;
...
public final class ForgeCompatibilityMixinPlugin implements IMixinConfigPlugin {
	private static final String MIXIN_PACKAGE = "RailOptimization.forge.mixin.";
	...
	public void postApply(String targetClassName, ClassNode targetClass, String mixinClassName, IMixinInfo mixinInfo) {
		switch (mixinClassName) {
			...
			default -> throw new IllegalStateException("Unknown Forge compatibility mixin: " + mixinClassName);
		}
	}
		...
		for (MethodNode method : targetClass.methods) {
			...
			for (var instruction : method.instructions.toArray()) {
				if (!(instruction instanceof MethodInsnNode call) || !call.owner.equals(BLOCK_STATE)
						|| !call.desc.equals(CONDUCTOR_DESCRIPTOR)) continue;
	...
	private static void replaceCategoryChecks(ClassNode targetClass, Set<String> methodNames) {
		...
		for (MethodNode method : targetClass.methods) {
			...
			for (var instruction : method.instructions.toArray()) {
				if (!(instruction instanceof MethodInsnNode call) || !call.owner.equals(BLOCK_STATE)
						|| !call.desc.equals("(Lnet/minecraft/world/level/block/Block;)Z")) continue;
				call.setOpcode(Opcodes.INVOKESTATIC);
				call.owner = "RailOptimization/forge/ForgeRailCompatibility";
	...
	private static void replaceNotificationSources(ClassNode targetClass) {
		...
		for (MethodNode method : targetClass.methods) {
			...
			source.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "RailOptimization/forge/ForgeRailCompatibility", "notificationSource",
					"(L" + BLOCK_STATE + ";)Lnet/minecraft/world/level/block/Block;", false));
